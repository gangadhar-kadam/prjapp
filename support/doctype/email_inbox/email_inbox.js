...
erpnext.support.EmailInbox = wn.ui.form.Controller.extend({
	customer: function() {
		var me = this;
...
$.extend(cur_frm.cscript, new erpnext.support.EmailInbox({frm: cur_frm}));
...
$.extend(cur_frm.cscript, {
	...
	set_flag:function(doc)
	{
	//console.log("in the set_flag")
	return wn.call({
			method:"support.doctype.email_inbox.email_inbox.set_flag",
			args: {
	...
	set_status: function(status) {
		return wn.call({
			method:"support.doctype.email_inbox.email_inbox.set_status",
			args: {
				name: cur_frm.doc.name,
				status: status
			},
